...
import { AntendeeRepository } from "./AntendeeRepository.js";

describe("AntendeeRepository", () => {
  describe("AntendeeRepository constructor - hallMap", () => {
    it("starts with no halls when none are provided", () => {
      const hallManager = new AntendeeRepository();

      expect(hallManager.getHallPoolSize()).toBe(0);
    });
    ...
    it("registers provided halls", () => {
      ...
      const hallManager = new AntendeeRepository({ halls });

      expect(hallManager.getHallPoolSize()).toBe(halls.length);
  ...
  describe("AntendeeRepository registerToHallWithSpecificAttendeeId", () => {
    // Các hall này nằm đâu vì sau mỗi lần chạy, cần làm mới danh sách Attendee
    let firstHall: Hall, secondHall: Hall, hallManager: AntendeeRepository;
    ...
    beforeEach(() => {
      ...
      hallManager = new AntendeeRepository({ halls: [firstHall, secondHall] });
    });
    ...
    it("return null when no attendee found in any halls.", () => {
      ...
      const foundHallId = hallManager.registerToHallWithAttendeeId(
        idToRegister,
    ...
    it("predicate returns one hall", () => {
      ...
      const foundHall = hallManager.registerToHallWithAttendeeId(
        "attendee_1",
        attendeeNeedRegister,
        predicate,
      );
    ...
    it.each([
      ...
      expect(() =>
        hallManager.registerToHallWithAttendeeId(
          attendeeUUID,
          attendeeNeedRegister,
          predicate,
        ),
